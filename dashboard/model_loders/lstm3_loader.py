...
_lstm3_model = None
_lstm3_vocab = None
_lstm3_label_classes = None
_lstm3_device = None


def load_lstm3_model(force_reload: bool = False) -> Tuple[nn.Module, Dict[str, int], List[str], torch.device]:
    """
    Carga el modelo LSTM 3 desde el archivo guardado.
    ...
    """
    global _lstm3_model, _lstm3_vocab, _lstm3_label_classes, _lstm3_device

    # Si ya esta cargado y no se fuerza recarga, devolver cache
    if not force_reload and _lstm3_model is not None:
        return _lstm3_model, _lstm3_vocab, _lstm3_label_classes, _lstm3_device

    # Ruta al modelo
    current_dir = Path(__file__).parent.parent.parent
    model_path = current_dir / "snapshots" / "lstm" / "lstm_balanced_focal_best.pt"
    ...
    # Parametros del modelo (misma arquitectura que LSTM 2)
    ...
    _lstm3_model = model
    _lstm3_vocab = vocab
    _lstm3_label_classes = label_classes
    _lstm3_device = device

    return model, vocab, label_classes, device


def predict_with_lstm3(text: str) -> Dict[str, any]:
    """
    Realiza una prediccion usando el modelo LSTM 3.
    ...
    model, vocab, label_classes, device = load_lstm3_model()
...
def get_lstm3_info() -> Dict[str, any]:
    """
    Obtiene informacion sobre el modelo LSTM 3 sin cargarlo completamente.
    ...
    return {
        'model_name': 'LSTM_3',
        'model_type': 'LSTM Bidireccional con Atencion (Focal Loss)',
